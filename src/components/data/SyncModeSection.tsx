...
import { SYNC_MODE_DEFS } from "./mappingData";
import type { SyncMode } from "./connectionTypes";

/**
 * Sync-mode section (LATENT). Three radio-style option cards — realtime /
 * **daily (default, active)** / manual. Selecting one flips client state + toast
 * (handled by the parent). Built from the Simple POS syncModeDef + Taste.
 */
export function SyncModeSection({
  ...
  return (
    ...
    >
      <div className="mb-3">
        <div className="text-[14px] font-bold">โหมดการส่งบัญชี · Sync mode</div>
        <div className="text-[11.5px]" style={{ color: "#94a3b8" }}>
          เลือกว่าจะส่งเอกสารเข้าบัญชีเมื่อใด
        </div>
      </div>
      <div role="radiogroup" aria-label="โหมดการส่งบัญชี" className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
        {SYNC_MODE_DEFS.map((m) => {
